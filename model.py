...
from fpsa import FixedPointSelfAttention
...
class CausalFixedPointSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        assert (
        ...
        # Check for flash attention support and fail if not present
        if not hasattr(torch.nn.functional, "scaled_dot_product_attention"):
            raise RuntimeError(
                "Flash Attention is required but not available. Please upgrade to PyTorch >= 2.0."
            )

        # Regularization layer
        self.resid_dropout = nn.Dropout(config.dropout)

        # Initialize FixedPointSelfAttention with forced flash=True, causal=True
